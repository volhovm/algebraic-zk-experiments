...
#[test]
fn batch_verification_benchmark() {
    ...
    println!(
        "Step 1: Generating protocol state for {} nodes...",
        num_nodes
    );
    let generated_state = generate_random_state(&pp, num_nodes, &mut rng);
    ...
    println!(
        "\nStep 2: Generating {} messages with TTL {}...",
        NUM_MESSAGES, TTL
    );
    ...
    println!(
        "\nGenerated {} messages in {:.2} ms ({:.2} ms per message)",
        NUM_MESSAGES,
        spawn_time.as_secs_f64() * 1000.0,
        spawn_time.as_secs_f64() * 1000.0 / NUM_MESSAGES as f64
    );

    // Calculate total Schnorr signatures
    let total_schnorr_sigs: usize = all_messages.iter().map(|msg| msg.hop_count()).sum();
    println!(
        "Total Schnorr signatures across all messages: {}",
        total_schnorr_sigs
    );
    ...
    for &batch_size in &batch_sizes {
        ...
        // Take only the first batch_size messages
        let batch = &all_messages[0..batch_size];
        ...
        .expect("Batch verification error");
        let batch_time = verify_start.elapsed();

        assert!(valid, "Batch verification failed");

        // Calculate statistics
        let avg_time_per_message = batch_time.as_secs_f64() * 1000.0 / batch_size as f64;

        // Calculate Schnorr signatures for verified messages
        let verified_schnorr_sigs: usize = batch.iter().map(|msg| msg.hop_count()).sum();
        let avg_time_per_schnorr = batch_time.as_secs_f64() * 1000.0 / verified_schnorr_sigs as f64;

        // Calculate throughput
        let throughput_msgs_per_sec = batch_size as f64 / batch_time.as_secs_f64();
        let throughput_schnorr_per_sec = verified_schnorr_sigs as f64 / batch_time.as_secs_f64();

        println!("  Messages verified: {}", batch_size);
        println!("  Total Schnorr signatures: {}", verified_schnorr_sigs);
        println!("  Total time: {:.2} ms", batch_time.as_secs_f64() * 1000.0);
        println!("  Average time per message: {:.2} ms", avg_time_per_message);
        println!("  Average time per Schnorr: {:.2} ms", avg_time_per_schnorr);
        println!(
            "  Throughput: {:.2} messages/sec ({:.2} Schnorr/sec)",
            throughput_msgs_per_sec, throughput_schnorr_per_sec
        );
        println!();
    }
    ...
    println!(
        "Batch Size | Total time (ms) | Avg ms/message | Avg ms/Schnorr | Throughput (msgs/sec)"
    );
    println!(
        "-----------|-----------------|----------------|----------------|---------------------"
    );
    ...
    for &batch_size in &batch_sizes {
        ...
        println!(
            "{:10} | {:15.2} | {:14.2} | {:14.2} | {:19.2}",
            batch_size,
            batch_time.as_secs_f64() * 1000.0,
            avg_time_per_message,
            avg_time_per_schnorr,
            throughput
        );
    }
