...
impl<E: Pairing, QAP: R1CSToQAP> Groth16<E, QAP> {
    ...
    /// Batch verify multiple Groth16 proofs across different circuits.
    ///
    /// This function batches verification of proofs from different circuits by combining
    /// all pairing checks into a single multi_miller_loop and final exponentiation.
    /// Given proofs π_1, ..., π_n with potentially different verification keys,
    /// we sample random coefficients r_1, ..., r_n and verify:
    ///
    /// ∏ e(rᵢ·Aᵢ, Bᵢ) = ∏ e(rᵢ·αᵢ, βᵢ) · ∏ e(rᵢ·prepared_inputsᵢ, -γᵢ) · ∏ e(rᵢ·Cᵢ, -δᵢ)
    ///
    /// This is rearranged for efficient computation using a single multi_miller_loop:
    /// e(∑ rᵢ·Aᵢ, Bᵢ) · e(∑ rᵢ·prepared_inputsᵢ, -γᵢ) · e(∑ rᵢ·Cᵢ, -δᵢ) = ∏ e(rᵢ·αᵢ, βᵢ)
    ///
    /// # Arguments
    /// * `proofs_pvks_and_inputs` - Vec of (proof, pvk, prepared_inputs) tuples, where each
    ///   proof can use a different verification key
    ///
    /// # Returns
    /// `Ok(true)` if all proofs are valid, `Ok(false)` otherwise
    pub fn batch_verify_proofs_cross_circuit(
        proofs_pvks_and_inputs: &[(Proof<E>, &PreparedVerifyingKey<E>, E::G1)],
    ) -> R1CSResult<bool> {
        use ark_std::rand::thread_rng;

        if proofs_pvks_and_inputs.is_empty() {
            return Ok(true);
        }

        // For single proof, use regular verification
        if proofs_pvks_and_inputs.len() == 1 {
            return Self::verify_proof_with_prepared_inputs(
                proofs_pvks_and_inputs[0].1,
                &proofs_pvks_and_inputs[0].0,
                &proofs_pvks_and_inputs[0].2,
            );
        }

        let mut rng = thread_rng();
        let mut random_coeffs = Vec::with_capacity(proofs_pvks_and_inputs.len());
        for _ in 0..proofs_pvks_and_inputs.len() {
            random_coeffs.push(E::ScalarField::rand(&mut rng));
        }

        // Prepare G1 elements for left side of pairing equation
        // For each proof i: rᵢ·Aᵢ, rᵢ·prepared_inputsᵢ, rᵢ·Cᵢ
        let mut g1_elements = Vec::with_capacity(proofs_pvks_and_inputs.len() * 3);

        for ((proof, _, prepared_inputs), coeff) in
            proofs_pvks_and_inputs.iter().zip(random_coeffs.iter())
        {
            // Add rᵢ·Aᵢ
            let scaled_a = proof
                .a
            ...
            // Add rᵢ·prepared_inputsᵢ
            let scaled_inputs = prepared_inputs
                .mul_bigint(coeff.into_bigint())
                .into_affine();
            g1_elements.push(<E::G1Affine as Into<E::G1Prepared>>::into(scaled_inputs));
            ...
            let scaled_c = proof
                ...
                .mul_bigint(coeff.into_bigint())
                .into_affine();
            g1_elements.push(<E::G1Affine as Into<E::G1Prepared>>::into(scaled_c));
        }

        // Prepare G2 elements for left side of pairing equation
        // For each proof i: Bᵢ, -γᵢ, -δᵢ
        let mut g2_elements = Vec::with_capacity(proofs_pvks_and_inputs.len() * 3);

        for (proof, pvk, _) in proofs_pvks_and_inputs.iter() {
            // Add Bᵢ
        ...
        // Perform left side multi_miller_loop
        let lhs_ml = E::multi_miller_loop(g1_elements, g2_elements);

        // Compute right hand side: ∏ e(rᵢ·αᵢ, βᵢ)
        // We compute this by scaling each α_g1_beta_g2 by rᵢ
        // Start with the first element
        let mut rhs = proofs_pvks_and_inputs[0]
            .1
            .alpha_g1_beta_g2
            .pow(random_coeffs[0].into_bigint());

        // Multiply by the rest
        for ((_, pvk, _), coeff) in proofs_pvks_and_inputs
            .iter()
            .zip(random_coeffs.iter())
            .skip(1)
        {
            rhs *= pvk.alpha_g1_beta_g2.pow(coeff.into_bigint());
        }

        // Final exponentiation on left side
        let lhs = E::final_exponentiation(lhs_ml).unwrap();
