...
use crate::proving::relations::curve::*;
use crate::proving::relations::lookup::*;
...
mod tests {
    ...
    #[test]
    fn test_blsjubjub_randomize() {
        let mut rng = rand::thread_rng();
        let c = JubjubA::rand(&mut rng);
        let h = JubjubA::rand(&mut rng);
        let r: JubJubScalar = <JubjubA as AffineRepr>::ScalarField::rand(&mut rng);
        let blinding = h * r;
        ...
        let ver_c_x_var = verifier.allocate(None).unwrap();
        let ver_c_y_var = verifier.allocate(None).unwrap();
        let ver_c_x_tilde_var = verifier.allocate(None).unwrap();
        let ver_c_y_tilde_var = verifier.allocate(None).unwrap();
        ...
        re_randomize::<_, _, JubjubConfig, _>(
            ...
            ver_c_x_tilde_var.into(),
            ver_c_y_tilde_var.into(),
            None,
        );
    ...
    #[test]
    fn test_elemwise_randomize() {
        let mut rng = rand::thread_rng();
        let g = JubjubA::rand(&mut rng);
        println!("g {:?}", g);
        println!("gx {}", g.x);
        println!("gy {}", g.y);

        let sk: JubJubScalar = <JubjubA as AffineRepr>::ScalarField::from(2);

        println!("sk {:?}", sk);
        let b = (g * sk).into_affine();
        println!("b {:?}", b);
        println!("bx {}", b.x);
        println!("by {}", b.y);
        ...
        /*let numerator = g.x.square() * <JubjubA as AffineRepr>::BaseField::from(3u64);     // 3 * x^2
        let denominator = g.y.double();                               // 2*y
        let lambda = numerator * denominator.inverse().unwrap();     // slope = (3x^2) / (2y)

        let x2 = lambda.square() - g.x.double();
        let y2 = lambda * (g.x - x2) - g.y;*/

        let a = JubjubConfig::COEFF_A;

        // λ = (3 x^2 + A) / (2y)
        let numerator = g.x.square() * <JubjubA as AffineRepr>::BaseField::from(3u64) + a;
        let denominator = g.y.double().inverse().unwrap();
        let lambda: <JubjubA as AffineRepr>::BaseField = numerator * denominator;
    ...
    fn test_understand_randomize() {
        ...
        let sk: PallasScalar = <PallasA as AffineRepr>::ScalarField::rand(&mut rng);
        let c = (g * sk).into_affine();
        let r: PallasScalar = <PallasA as AffineRepr>::ScalarField::rand(&mut rng);
        let blinding = h * r;
        let c_tilde = (c + blinding).into_affine();
        println!("ctilde x: {:?}", c_tilde.x().unwrap());
        ...
        let proof = {
            ...
            println!("metrics {:?}", prover.metrics());
            re_randomize(
                &mut prover,
                &tables,
                PointRepresentation {
    ...
    fn test_jj_tables() {
        ...
        let tables = build_tables(h);
        let lambda = <JubJubScalar as PrimeField>::MODULUS_BIT_SIZE as usize;
        let m = lambda / 3 + 1;
        let r_bigint: <JubJubScalar as PrimeField>::BigInt = r.into();
        let random_bits = r_bigint.to_bits_le();
        let mut h_r_acc = JubjubA::zero();
        for i in 1..m + 1 {
            // n.b. i is 0 indexed
            let table = tables[i - 1];
            let bi = (i - 1) * 3;
